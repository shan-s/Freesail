 ...
 */
export function buildCatalogConfig(packagePath: string, srcPath: string, framework: 'react' | 'lit' = 'react'): CatalogConfig {
  const name = path.basename(packagePath);
  const prefix = name.replace(/[-_]catalog(?:-lit|-react)?$/, '').replace(/-/g, '_');
  return { name, packagePath, srcPath, prefix, framework };
}
...
export function buildConfigFromEntry(packageDir: string, entry: Record<string, unknown>): CatalogConfig {
  ...
  return {
    name, packagePath: packageDir, srcPath, prefix, framework,
 ...
 *
 * The optional trailing `-lit`/`-react` framework qualifier is stripped along
 * with `-catalog` so that e.g. @freesail/standard-catalog-lit derives the same
 * prefix as @freesail/standard-catalog — both export the same-shaped
 * `StandardCatalog` constant (they implement the same catalogId/schema, just
 * for different renderers), so the generated `generated-includes.ts` import
 * must reference the same constant name regardless of which one is included.
 *
 * Examples:
 *   @freesail/standard-catalog      →  standard
 *   @freesail/standard-catalog-lit  →  standard
...
export function derivePackagePrefix(packageName: string): string {
  ...
  const withoutSuffix = withoutScope.replace(/-catalog(?:-lit|-react)?$/, '');
 ...
 *   };
 */
function generateIncludesTs(includes: CatalogInclude, srcPath: string, framework: 'react' | 'lit'): void {
  const lines: string[] = [
  ...
  const componentMapType = framework === 'lit' ? 'Record<string, FreesailComponent>' : 'Record<string, React.ComponentType<FreesailComponentProps>>';
  ...
  lines.push(`export const includedComponents: ${componentMapType} = {`);
...
export function prepareCatalog(config: CatalogConfig): boolean {
  ...
  if (includeJson) {
    generateIncludesTs(includeJson, config.srcPath, config.framework);
