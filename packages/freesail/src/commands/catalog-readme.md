...
src/
  ...
  {{prefix}}_catalog.json  # Generated catalog (do not edit manually)
```
...
import { {{pascalPrefix}}Catalog } from '{{prefix}}_catalog';

function App() {
  return (
    <FreesailProvider catalogs={[{{pascalPrefix}}Catalog]}>
      {/* your app */}
    </FreesailProvider>
  );
}
