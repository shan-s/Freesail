...
src/
  ...
  weather_catalog.json  # Generated catalog (do not edit manually)
```
...
| Command | Description |
| --- | --- |
| `npx freesail prepare catalog` | Merge schemas and generate `weather_catalog.json` |
...
import { WeatherCatalog } from 'weather_catalog';

function App() {
  return (
    <FreesailProvider catalogs={[WeatherCatalog]}>
      {/* your app */}
    </FreesailProvider>
  );
}
