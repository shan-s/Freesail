/**
 * @fileoverview Standard Catalog Functions
 *
...
import { setComponentState, type FunctionImplementation } from '@freesail/core';
...
export const regex: FunctionImplementation = (value: unknown, pattern: string) => {
  ...
  try {
    ...
    if (performance.now() - start > 50) {
      console.warn('[functions] regex: pattern took >50ms, consider simplifying:', pattern);
    }
    return matched;
  } catch {
    return false;
