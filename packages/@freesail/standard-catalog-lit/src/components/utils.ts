 ...
 * Exported publicly via the `@freesail/standard-catalog-lit/utils` subpath.
 * Framework-agnostic — identical logic to @freesail/standard-catalog's utils.ts,
 * just typed against a plain CSS-declaration map instead of React's CSSProperties.
 */

/** A plain CSS declaration map, as consumed by lit-html's styleMap() directive. */
...
export function mapJustify(justify: string | undefined): string {
  switch (justify) {
